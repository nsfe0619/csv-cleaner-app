...
export class CsvCleanerService {
  ...
  // ✅ 格式化電話號碼為 `+886-XXX-XXX-XXX`
  private formatPhone(phone: string): string {
    let digits = phone.replace(/\D/g, '');
    if (digits.length === 0) return 'INVALID'; // ✅ 空值變 "INVALID"
    if (digits.length < 10) return '+886-000-000-000'; // ✅ 不足 10 碼填 0
    digits = digits.slice(-10);
    return `+886-${digits.slice(1, 4)}-${digits.slice(4, 7)}-${digits.slice(
      7
    )}`;
  }
  ...
  private removeNonDigits(input: string): string {
    return input.replace(/\D/g, ''); // 只保留數字
  }

  private maskSensitiveData = (data: string): string => {
    const cleanData = this.removeNonDigits(data);
    if (cleanData.length < 16) return "INVALID";

    return `****-****-****-${cleanData.slice(-4)}`;
  };
